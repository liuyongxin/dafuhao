...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _backGroundView = [[UIView alloc]initWithFrame:CGRectMake(0,0 , DFHScreenW, DFHScreenH)];
        _backGroundView.backgroundColor = [UIColor blackColor];
        _backGroundView.alpha = 0.05;
        [self addSubview:_backGroundView];

        _popUpView = [[UIView alloc]initWithFrame:frame];
        _popUpView.backgroundColor = [UIColor grayColor];
        [self addSubview:_popUpView];

        _popUpView.layer.cornerRadius = 6.0f;
        _popUpView.layer.shadowOffset = CGSizeMake(2, 2);
        _popUpView.layer.shadowRadius = 2.0f;
        _popUpView.layer.shadowOpacity = 0.4f;
        ...
        [_popUpView addSubview:highlight];
        ...
        [_popUpView addSubview:prevButton];
        ...
        [_popUpView addSubview:nextButton];
        ...
        [_popUpView addSubview:calendarContainer];
...
- (void)layoutSubviews {
    ...
    CGFloat containerWidth = _popUpView.bounds.size.width - (CALENDAR_MARGIN * 2);
    ...
    self.highlight.frame = CGRectMake(1, 1, _popUpView.bounds.size.width - 2, 1);

    self.titleLabel.frame = CGRectMake(0, 0, _popUpView.bounds.size.width, TOP_HEIGHT);
    self.prevButton.frame = CGRectMake(BUTTON_MARGIN, BUTTON_MARGIN, 48, 38);
    self.nextButton.frame = CGRectMake(_popUpView.bounds.size.width - 48 - BUTTON_MARGIN, BUTTON_MARGIN, 48, 38);
...
- (void)show:(BOOL )animated
{
    ...
    [self showInView:keyWindow animated:animated];
...
- (void)showAnimation  //仿UIAlertView动画效果
{
    ...
    popAnimation.timingFunctions = @[[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut],
                                     ...
                                     [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
    [self.popUpView.layer addAnimation:popAnimation forKey:@"showAnimation"];
}
...
- (void)dismissAnimation
{
    ...
    [self.popUpView.layer addAnimation:scaleAnimation forKey:@"scaleAnimation"];
}

-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag //动画代理
{
    if(anim ==[self.popUpView.layer animationForKey:@"scaleAnimation"])
    {
        [self.popUpView.layer removeAnimationForKey:@"scaleAnimation"];
        [self removeFromSuperview];
    }
    else if (anim == [self.popUpView.layer animationForKey:@"showAnimation"])
    {
        [self.popUpView.layer removeAnimationForKey:@"showAnimation"];
    }
}
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if (!CGRectContainsPoint(self.popUpView.frame, point)) {
        [self dismiss:YES];
    }
}
