...
+ (void)drawStringInRect2:(BOOL)isbold fontsize:(int)fontsize isvercenter:(BOOL)isvercenter str:(NSString *)str rect:(CGRect)rect alignment:(NSTextAlignment)alignment clr:(UIColor *)clr
{
    if (str != nil)
	{
		...
		if (isbold)
            ...
            font = fontsize == 0 ? [UIFont systemFontOfSize:16.] : [UIFont systemFontOfSize:fontsize];
...
{
    CGSize size = [str sizeWithFont:font constrainedToSize:CGSizeMake(rect.size.width, 300) lineBreakMode:NSLineBreakByWordWrapping];
    int lines = size.height / font.lineHeight;
    
    CGRect curRt = isvercenter ? CGRectMake(rect.origin.x, rect.origin.y + (rect.size.height - (font.xHeight + font.capHeight)*(lines==0?1:lines)) * 0.5, rect.size.width, (font.xHeight + font.capHeight)*(lines==0?1:lines)) : rect;
    [color set];
